...
use crate::registry::{IdentifiableType, Registry};
use crate::{HandleSubscription, LifecycleMessage, ObjectId};
    ...
    TowerService<RequestEnvelope> for Service<S, P>
{
    ...
    fn call(&mut self, req: RequestEnvelope) -> Self::Future {
        ...
        let result = async move {
            ...
            address_mismatch.map_or(Ok(()), |err| Err(err))?;
...
impl Stream for SubscriptionResponseIter {
    ...
    fn poll_next(
        ...
        if this._count >= 3 {
            ...
            let poll = std::task::Poll::Ready(Some(SubscriptionResponse { body: Ok(vec![]) }));
            poll
        }
    }
}
        ...
        let result = async move {
            ...
            let response = this
                ...
                .send(
                    &req.handler_type,
                    &req.handler_id,
                    &HandleSubscription::user_defined_type_id(),
                    &[],
                    this.app_data.clone(),
                )
                .await;
            ...
            let _channel = match response {
                // WIP
                // I think this here should be an async channel,
                // then the channel needs to be passed to SubscriptionResponseIter
                Ok(body) => {
                    println!("[TODO] Body: {:?}", body);
                    Ok(())
                }
                Err(err) => {
                    println!("[TODO] HandlerError: {:?}", err);
                    Err(err)
                }
            }?;

            tokio::time::sleep(Duration::from_secs(1)).await;
            Ok(SubscriptionResponseIter::default())
        };
        Box::pin(result)
    }
}

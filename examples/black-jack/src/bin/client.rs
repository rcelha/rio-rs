...
async fn main() -> anyhow::Result<()> {
    ...
    while let Some(data) = subscription.next().await {
        if let Ok(ref v) = data {
            //
            if let Some(GameServerResponse::State(
                table_state,
                current_player,
                player_list,
                hands,
            ...
            )) = v.0.last()
            {
                    ...
                    for i in 0..player_list.len() {
                        ...
                        match i_result {
                            ...
                            black_jack::game_server::Winner::Tie => {
                                println!("The player {} has tied with the DEALER", i_player_id);
                            }
                        }
                    }
                    exit(0);
                ...
                if current_player.is_empty() {
                    ...
                    loop {
                        ...
                        command.clear();
                        let timeout_result = tokio::time::timeout(Duration::from_secs(9), async {
                        ...
                        let msg =
                            PlayerCommand(GameServerRequest::Player(user_id.clone(), command));

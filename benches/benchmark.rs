...
async fn channel_registry_handle(
    options: &(rio_rs::channel_registry::Registry, Arc<AppData>, usize),
) {
...
async fn registry_handle(options: &(Registry, Arc<AppData>, usize)) {
    let (r, app_data, n) = options;
    for i in 1..*n {
        let response = r
            .send(
...
pub fn registry_benchmark(c: &mut Criterion) {
    ...
    for size in [1, 100, 10_000] {
        // channel_registry
        {
            let mut options = runtime.block_on(async {
